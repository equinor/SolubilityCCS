...
This script analyzes the corrosion potential of CO2 systems containing acids (H2SO4 or HNO3)
and water by calculating phase behavior and liquid phase formation.
"""
...
def analyze_corrosion_potential(acid="H2SO4", acid_in_co2=10, water_in_co2=10.0, 
                               temperature=2, pressure=60, flow_rate=100):
    """
    ...
    if fluid.betta < 1:
        results.update({
            ...
            'water_in_liquid_mol_frac': fluid.phases[1].get_component_fraction("H2O"),
...
def print_analysis_results(results, acid):
    ...
    print(f"Mole fraction of gas phase to total phase {results['gas_phase_fraction']:.16f} mol/mol")
    ...
    if 'liquid_phase_name' in results:
        ...
        print(f"Liquid phase formed {results['liquid_flow_rate_ty']:.13f} t/y")
        print(f"Water in liquid phase {results['water_in_liquid_mol_frac']:.17f} mol fraction")
        print(f"{acid} in liquid phase {results['acid_in_liquid_mol_frac']:.16f} mol fraction")
